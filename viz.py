...
def plot_loss_metrics(path, history, step=None):
    fig = plt.figure()
    plt.plot(history['train']['epoch'], history['train']['err'],
             color='b', label='training')
    plt.plot(history['val']['epoch'], history['val']['err'],
    ...
    if step is None:
        ...
        fig.savefig(os.path.join(path, 'loss_step_{}.png'.format(step)), dpi=200)
    plt.close('all')
    ...
    if step is None:
        ...
        fig.savefig(os.path.join(path, 'metrics_step_{}.png'.format(step)), dpi=200)
    plt.close('all')
